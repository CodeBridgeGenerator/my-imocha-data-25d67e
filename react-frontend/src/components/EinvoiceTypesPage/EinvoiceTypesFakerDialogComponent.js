...
import eInvoiceTypesFakerFactory from "./eInvoiceTypesFakerFactory";
...
const EInvoiceTypesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = eInvoiceTypesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("eInvoiceTypes").create(elem));
...
export default connect(mapState, mapDispatch)(EInvoiceTypesFakerDialogComponent);
