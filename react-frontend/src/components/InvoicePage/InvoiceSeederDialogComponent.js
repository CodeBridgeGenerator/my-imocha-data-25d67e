...
import seederData from "./invoice.seeder.json";
...
const InvoiceSeederDialogComponent = (props) => {
  ...
  const onRun = async () => {
    ...
    try {
      for (let i = 0; i < seederData.length; i++) {
        ...
        const res = await client.service("invoice").create(_data);
        results = [...results, res];
      }
      props.onHide();
      props.alert({
...
export default connect(mapState, mapDispatch)(InvoiceSeederDialogComponent);
