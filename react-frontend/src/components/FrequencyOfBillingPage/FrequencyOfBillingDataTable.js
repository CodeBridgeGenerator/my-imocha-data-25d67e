...
const FrequencyOfBillingDataTable = ({
  items,
  fields,
  onEditRow,
  onRowDelete,
  ...
  const pTemplate0 = (rowData, { rowIndex }) => (
    <p>{rowData.frequencyofbilling}</p>
  );
  ...
  return (
    <>
      ...
      <Dialog
        header="Upload FrequencyOfBilling Data"
        visible={showUpload}
        onHide={() => setShowUpload(false)}
      >
        <UploadService
          user={user}
          serviceName="frequencyOfBilling"
          onUploadComplete={() => {
      ...
      <Dialog
        header="Search FrequencyOfBilling"
