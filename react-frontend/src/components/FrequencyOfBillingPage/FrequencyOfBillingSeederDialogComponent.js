...
import seederData from "./frequencyOfBilling.seeder.json";
...
const FrequencyOfBillingSeederDialogComponent = (props) => {
  ...
  const onRun = async () => {
    ...
    try {
      for (let i = 0; i < seederData.length; i++) {
        ...
        const res = await client.service("frequencyOfBilling").create(_data);
        results = [...results, res];
      }
      props.onHide();
      props.alert({
