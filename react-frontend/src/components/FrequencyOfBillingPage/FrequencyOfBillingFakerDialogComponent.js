...
import frequencyOfBillingFakerFactory from "./frequencyOfBillingFakerFactory";
...
const FrequencyOfBillingFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = frequencyOfBillingFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("frequencyOfBilling").create(elem),
    );
