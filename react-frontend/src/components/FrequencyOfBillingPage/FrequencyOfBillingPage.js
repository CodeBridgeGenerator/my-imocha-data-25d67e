...
import FrequencyOfBillingDatatable from "./FrequencyOfBillingDataTable";
import FrequencyOfBillingEditDialogComponent from "./FrequencyOfBillingEditDialogComponent";
import FrequencyOfBillingCreateDialogComponent from "./FrequencyOfBillingCreateDialogComponent";
import FrequencyOfBillingFakerDialogComponent from "./FrequencyOfBillingFakerDialogComponent";
import FrequencyOfBillingSeederDialogComponent from "./FrequencyOfBillingSeederDialogComponent";

const FrequencyOfBillingPage = (props) => {
  ...
  const filename = "frequencyOfBilling.csv";

  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("frequencyOfBilling");
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((error) => {
        ...
        setLoading(false);
        props.alert({
          title: "Frequency Of Billing",
          type: "error",
          message: error.message || "Failed get Frequency Of Billing",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("frequencyOfBilling").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("frequencyOfBilling").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/frequencyOfBilling/${data._id}`);
  };
    ...
    <div className="mt-5">
      <div className="grid">
        <div className="col-6 flex justify-content-start">
          <h3 className="mb-0 ml-2">Frequency Of Billing </h3>
        </div>
        <div className="col-6 flex justify-content-end">
          <>
            <Button
              ...
              role="frequencyOfBilling-add-button"
            />
            <SplitButton
      ...
      <div className="grid align-items-center">
        <div className="col-12" role="frequencyOfBilling-datatable">
          <FrequencyOfBillingDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(FrequencyOfBillingPage);
