...
import identifyTypeFakerFactory from "./identifyTypeFakerFactory";
...
const IdentifyTypeFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = identifyTypeFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("identifyType").create(elem),
    );
...
export default connect(mapState, mapDispatch)(IdentifyTypeFakerDialogComponent);
