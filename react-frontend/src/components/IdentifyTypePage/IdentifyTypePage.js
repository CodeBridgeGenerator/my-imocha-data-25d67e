...
import IdentifyTypeDatatable from "./IdentifyTypeDataTable";
import IdentifyTypeEditDialogComponent from "./IdentifyTypeEditDialogComponent";
import IdentifyTypeCreateDialogComponent from "./IdentifyTypeCreateDialogComponent";
import IdentifyTypeFakerDialogComponent from "./IdentifyTypeFakerDialogComponent";
import IdentifyTypeSeederDialogComponent from "./IdentifyTypeSeederDialogComponent";

const IdentifyTypePage = (props) => {
  ...
  const filename = "identifyType.csv";

  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("identifyType");
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Identify Type",
          type: "error",
          message: error.message || "Failed get Identify Type",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("identifyType").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("identifyType").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/identifyType/${data._id}`);
  };
    ...
    <div className="mt-5">
      <div className="grid">
        <div className="col-6 flex justify-content-start">
          <h3 className="mb-0 ml-2">Identify Type </h3>
        </div>
      ...
      <div className="grid align-items-center">
        <div className="col-12" role="identifyType-datatable">
          <IdentifyTypeDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(IdentifyTypePage);
