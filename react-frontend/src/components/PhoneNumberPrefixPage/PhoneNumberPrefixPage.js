...
import PhoneNumberPrefixDatatable from "./PhoneNumberPrefixDataTable";
...
import PhoneNumberPrefixFakerDialogComponent from "./PhoneNumberPrefixFakerDialogComponent";
import PhoneNumberPrefixSeederDialogComponent from "./PhoneNumberPrefixSeederDialogComponent";

const PhoneNumberPrefixPage = (props) => {
  ...
  const filename = "phoneNumberPrefix.csv";

  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("phoneNumberPrefix");
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get Phone Number Prefix",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("phoneNumberPrefix").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("phoneNumberPrefix").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/phoneNumberPrefix/${data._id}`);
  };
    ...
    <div className="mt-5">
      <div className="grid">
        <div className="col-6 flex justify-content-start">
          <h3 className="mb-0 ml-2">Phone Number Prefix </h3>
        </div>
        <div className="col-6 flex justify-content-end">
          <>
            <Button
              ...
              role="phoneNumberPrefix-add-button"
            />
            <SplitButton
      ...
      <div className="grid align-items-center">
        <div className="col-12" role="phoneNumberPrefix-datatable">
          <PhoneNumberPrefixDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(PhoneNumberPrefixPage);
