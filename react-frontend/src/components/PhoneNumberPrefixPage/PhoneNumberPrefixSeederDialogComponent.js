...
import seederData from "./phoneNumberPrefix.seeder.json";
...
const PhoneNumberPrefixSeederDialogComponent = (props) => {
  ...
  const onRun = async () => {
    ...
    try {
      for (let i = 0; i < seederData.length; i++) {
        ...
        const res = await client.service("phoneNumberPrefix").create(_data);
        results = [...results, res];
      }
      props.onHide();
      props.alert({
