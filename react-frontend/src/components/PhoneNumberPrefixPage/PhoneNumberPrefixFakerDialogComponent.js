...
import phoneNumberPrefixFakerFactory from "./phoneNumberPrefixFakerFactory";
...
const PhoneNumberPrefixFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = phoneNumberPrefixFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("phoneNumberPrefix").create(elem),
    );
