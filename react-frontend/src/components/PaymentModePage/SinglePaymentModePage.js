...
const SinglePaymentModePage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      .service("paymentMode")
      .get(urlParams.singlePaymentModeId, {
        query: {
          $populate: [
            {
              path: "createdBy",
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "PaymentMode",
          type: "error",
          message: error.message || "Failed get paymentMode",
        });
      });
        ...
        <div className="card w-full">
          <div className="grid ">
            <div className="col-12 md:col-6 lg:col-3">
              <label className="text-sm text-primary">Payment mode</label>
              <p className="m-0 ml-3">{_entity?.paymentmode}</p>
            </div>
